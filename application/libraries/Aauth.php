...
/* coming with v3
----------------
 * captcha (hmm bi bakal?m)
 * parametre olarak array alma
 * stacoverflow
 * public id sini 0 a e?itleyip öyle kontrol yapabilirdik (oni bo?ver u?a?um)
 * lock_user (until parametrsi)
 * unlock_user
 * send_pm() in errounda receiver ve sender için ayr? errorlar olabilür
 * ddos protect olay?n? daha mant?kl? hale getür
 * geçici ban ve e-mail ile tkrar aktifle?time olay?
*/
/**
 * Coming with v2
 * -------------
 ...
 * tamam // login e ip aderesi de eklemek laz?m
 * list_users da grup_par verilirse ve adam?n birden fazla grubu varsa nolurkun? // bi denemek laz?m belki distinct ile düzelir
 * tamam // e?er grup silinmi?se kullan?c?lar? da o gruptan sil (fire)
 * tamam //	 ismember la is admine 2. parametre olarak user id ekle
 * tamam // kepp infos errors die bi?ey yap ajax requestlerinde silinir errorlar
 * tmam // user variables
 * tamam // sistem variables
 * tmam // user perms
 * tamam gibi // 4mysql index fulltext index??
 * tamam //delete_user dan sonra grup ve perms ler de silinmeli
 * login() içinde login'i do?ru ?ekilde olsa da yine de login attempt art?yo kesin düzeltilecek
 ...
 * Done staff v1
 * -----------
 * tamam hac? // control die bi fonksiyon yaz?p adam önce login omu?mu sonra da yetkisi var m? die kontrol et. yetkisi yoksa yönlendir ve aktivitiyi güncelle
 ...
 * san?r?m ?imdi tamam // hatal? giri?de otomatik süreli kilit
 * ??  tamam heral // mail ile bilgilendirme
 * tamam heral // activasyon emaili
 * tamam gibi // yerine email check // username check
 * tamamland? // public eri?imi
 * tamam // Private messsages
 * tamam össen // errorlar düzenlenecek hepisiiii
 * tamam ama engelleme ve limit olay? koymad?m. // pm için okundu ve göster, sil, engelle? die fonksiyonlar eklencek , gönderilen pmler, al?nan pmler, aras?ndaki pmler,
 * tamm// already existedleri info yap onlar error de?il hac?m
 *
/*
